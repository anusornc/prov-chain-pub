...
usage() {
  cat <<'EOF_USAGE'
Usage:
  benchmark-toolkit/scripts/provchain-fluree-campaign.sh smoke [options]
  benchmark-toolkit/scripts/provchain-fluree-campaign.sh full [options]
  benchmark-toolkit/scripts/provchain-fluree-campaign.sh run [options]
  benchmark-toolkit/scripts/provchain-fluree-campaign.sh status <campaign_id>

Modes:
  smoke   Run a 1 epoch / 1 iteration Docker smoke campaign.
  full    Run the standard 30 epoch / 10 iteration ProvChain vs Neo4j vs Fluree campaign.
...
case "${MODE}" in
  smoke)
    ...
    DEFAULT_ID="smoke_trace_supply1000_provchain-neo4j-fluree_n1_$(date -u +%Y%m%dT%H%M%SZ)"
    ;;
  full)
    EPOCHS_VALUE="30"
    ITERATIONS_VALUE="10"
    SKIP_PREFLIGHT_VALUE="false"
    DEFAULT_ID="$(date -u +%Y%m%d)_trace_supply1000_provchain-neo4j-fluree_n30"
    ;;
  run)
    EPOCHS_VALUE="${EPOCHS:-3}"
    ITERATIONS_VALUE="${ITERATIONS:-10}"
    SKIP_PREFLIGHT_VALUE="${SKIP_PREFLIGHT:-false}"
    DEFAULT_ID="$(date -u +%Y%m%d)_trace_supply1000_provchain-neo4j-fluree_n${EPOCHS_VALUE}"
    ;;
  status)
    CAMPAIGN_ID_VALUE="${1:-}"
