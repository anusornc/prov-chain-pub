...
usage() {
  cat <<'EOF_USAGE'
Usage:
  benchmark-toolkit/scripts/provchain-bulk-import-r002-campaign.sh smoke [options]
  benchmark-toolkit/scripts/provchain-bulk-import-r002-campaign.sh profile [options]
  benchmark-toolkit/scripts/provchain-bulk-import-r002-campaign.sh full [options]
  benchmark-toolkit/scripts/provchain-bulk-import-r002-campaign.sh run [options]
  benchmark-toolkit/scripts/provchain-bulk-import-r002-campaign.sh status <campaign_id>

Modes:
  smoke    Run a 1 epoch / 1 iteration R002 confidence smoke campaign.
  profile  Run a 3 epoch / 3 iteration R002 profile campaign.
  full     Run a 30 epoch / 10 iteration R002 campaign after smoke/profile pass.
  run      Run a custom R002 campaign using --epochs, --iterations, and optional --id.
...
Options for smoke/profile/full/run:
  ...
  --port-base <n>             Set R002 host port block. Defaults to 18380.
  --products <list>           Comma-separated products. Defaults to provchain,neo4j,fluree.
  --legacy-provchain-import   Use the old per-triple ProvChain import path for A/B checks.
...
run_campaign() {
  ...
  local export_dir="${12}"
  local products="${13}"
  local provchain_import_mode="${14}"
  ...
  printf '[campaign] R002 ProvChain import mode: %s\n' "${provchain_import_mode}"
  printf '[campaign] products: %s\n' "${products}"
  printf '[campaign] host ports: provchain=%s metrics=%s neo4j_http=%s neo4j_bolt=%s fluree=%s\n' \
    "${provchain_http_port}" \
    "${provchain_metrics_port}" \
    "${neo4j_http_port}" \
    "${neo4j_bolt_port}" \
    "${fluree_http_port}"
  printf '[campaign] Neo4j runtime: heap_initial=%s heap_max=%s pagecache=%s load_batch_size=%s\n' \
    "${neo4j_heap_initial}" \
    "${neo4j_heap_max}" \
    "${neo4j_pagecache}" \
  ...
  (
    ...
    EVIDENCE_ROLE="r002_bulk_turtle_import_experiment_not_primary_paper_evidence" \
...
PORT_BASE_VALUE="${R002_PORT_BASE:-18380}"
EXPORT_DIR_VALUE="${EXPORT_DIR:-}"
PRODUCTS_VALUE="${PRODUCTS:-provchain,neo4j,fluree}"
PROVCHAIN_IMPORT_MODE_VALUE="${PROVCHAIN_IMPORT_MODE:-bulk-turtle-single-block}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --products)
      ...
      shift 2
      ;;
    --legacy-provchain-import)
      PROVCHAIN_IMPORT_MODE_VALUE="legacy-per-triple"
      shift
      ;;
    --skip-preflight)
      SKIP_PREFLIGHT_VALUE="true"
...
IMPORT_ID_LABEL="bulk-r002"
if [ "${PROVCHAIN_IMPORT_MODE_VALUE}" = "legacy-per-triple" ]; then
  IMPORT_ID_LABEL="legacy-r002"
fi

if [ -z "${CAMPAIGN_ID_VALUE}" ]; then
  case "${MODE}" in
    smoke)
      CAMPAIGN_ID_VALUE="smoke_import_${CAMPAIGN_DATASET_LABEL}_provchain-${IMPORT_ID_LABEL}_n1_${DATE_UTC}"
      ;;
    profile|run)
      CAMPAIGN_ID_VALUE="${DATE_UTC}_import_${CAMPAIGN_DATASET_LABEL}_provchain-${IMPORT_ID_LABEL}_n${EPOCHS_VALUE}"
      ;;
    full)
      CAMPAIGN_ID_VALUE="${DATE_UTC}_import_${CAMPAIGN_DATASET_LABEL}_provchain-${IMPORT_ID_LABEL}_n30"
      ;;
  esac
fi

if [ -z "${EXPORT_DIR_VALUE}" ]; then
  EXPORT_DIR_VALUE="docs/benchmarking/data/reference/${CAMPAIGN_ID_VALUE}"
fi
