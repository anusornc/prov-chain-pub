...
usage() {
  cat <<'EOF_USAGE'
Usage:
  benchmark-toolkit/scripts/provchain-neo4j-fluree-graphdb-campaign.sh smoke [options]
  benchmark-toolkit/scripts/provchain-neo4j-fluree-graphdb-campaign.sh profile [options]
  benchmark-toolkit/scripts/provchain-neo4j-fluree-graphdb-campaign.sh full [options]
  benchmark-toolkit/scripts/provchain-neo4j-fluree-graphdb-campaign.sh run [options]
  benchmark-toolkit/scripts/provchain-neo4j-fluree-graphdb-campaign.sh status <campaign_id>
...
Options for smoke/profile/full/run:
  ...
  --port-base <n>             Set isolated host port block. Defaults to 18480.
...
run_campaign() {
  ...
  local graphdb_http_port="${GRAPHDB_TRACE_HTTP_PORT:-$((port_base + 20))}"
  local graphdb_repository="${GRAPHDB_REPOSITORY:-provchain_trace}"
  local graphdb_graph_iri="${GRAPHDB_GRAPH_IRI:-http://provchain.org/benchmark/graphdb/trace}"

  validate_campaign_id "${campaign_id}"
  check_docker

  printf '[campaign] starting %s\n' "${campaign_id}"
  printf '[campaign] graphdb host ports: provchain=%s metrics=%s neo4j_http=%s neo4j_bolt=%s fluree=%s graphdb=%s\n' \
    "${provchain_http_port}" \
  ...
  (
    ...
    EVIDENCE_ROLE="graphdb_comparator_reference_candidate" \
    ...
    GRAPHDB_TRACE_HTTP_PORT="${graphdb_http_port}" \
...
case "${MODE}" in
  smoke)
    ...
    DEFAULT_ID="smoke_trace_supply1000_provchain-neo4j-fluree-graphdb_n1_${DATE_UTC}"
    ;;
  profile)
    EPOCHS_VALUE="3"
    ITERATIONS_VALUE="3"
    ...
    DEFAULT_ID="${DATE_UTC}_trace_supply1000_provchain-neo4j-fluree-graphdb_n3"
    ;;
  full)
    EPOCHS_VALUE="30"
    ITERATIONS_VALUE="10"
    SKIP_PREFLIGHT_VALUE="${SKIP_PREFLIGHT:-false}"
    EXPORT_ENABLED_VALUE="${EXPORT_EVIDENCE:-true}"
    DEFAULT_ID="${DATE_UTC}_trace_supply1000_provchain-neo4j-fluree-graphdb_n30"
    ;;
  run)
    ...
    SKIP_PREFLIGHT_VALUE="${SKIP_PREFLIGHT:-false}"
    EXPORT_ENABLED_VALUE="${EXPORT_EVIDENCE:-true}"
    DEFAULT_ID="${DATE_UTC}_trace_supply1000_provchain-neo4j-fluree-graphdb_n${EPOCHS_VALUE}"
    ;;
  status)
    CAMPAIGN_ID_VALUE="${1:-}"
...
PORT_BASE_VALUE="${GRAPHDB_CAMPAIGN_PORT_BASE:-18480}"
...
if [ -z "${EXPORT_DIR_VALUE}" ]; then
  EXPORT_DIR_VALUE="docs/benchmarking/data/reference/trace_${DATASET_SLICE_VALUE}_provchain_neo4j_fluree_graphdb_n${EPOCHS_VALUE}_${DATE_UTC}"
fi
