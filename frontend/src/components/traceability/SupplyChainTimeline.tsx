...
const SupplyChainTimeline: React.FC<SupplyChainTimelineProps> = ({
  ...
  const processTimelineSteps = (steps: TraceStep[]): TimelineStep[] => {
    ...
    if (filterParticipant !== "all") {
      filteredSteps = filteredSteps.filter(
        (step) => step.participant === filterParticipant,
      );
    }
  ...
  const getStepColor = (action: string): string => {
    ...
    if (actionLower.includes("production") || actionLower.includes("created"))
      return "#10b981";
    if (
      actionLower.includes("processing") ||
      actionLower.includes("manufactured")
    )
      return "#3b82f6";
    if (actionLower.includes("transport") || actionLower.includes("shipped"))
      return "#f59e0b";
    if (actionLower.includes("quality") || actionLower.includes("tested"))
      return "#8b5cf6";
    if (actionLower.includes("transfer") || actionLower.includes("ownership"))
      return "#ef4444";
  ...
  const renderTimeline = () => {
    ...
    const g = svg
      ...
      .attr(
        "transform",
        `translate(${config.margin.left}, ${config.margin.top}) scale(${zoomLevel})`,
      );
    ...
    if (timelineSteps.length > 1) {
      for (let i = 0; i < timelineSteps.length - 1; i++) {
        ...
        g.append("polygon")
          .attr(
            "points",
            `${midX - 5},${current.y_position - 3} ${midX + 5},${current.y_position} ${midX - 5},${current.y_position + 3}`,
          )
          .attr("fill", "#9ca3af")
          .style("opacity", 0.7);
      }
  ...
  const getUniqueParticipants = (): string[] => {
    ...
    const participants = new Set(
      timelineData.trace_path.map((step) => step.participant),
    );
  ...
  if (traceError) {
    return <Alert variant="error" message={traceError} />;
  }
      ...
      <Card className="p-6">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Badge variant="info">{timelineSteps.length} steps</Badge>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <button
                    onClick={() =>
                      onParticipantSelect?.(selectedStep.participant)
                    }
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
              {timelineSteps.length > 0
                ? Math.ceil(
                    (new Date(
                      timelineSteps[timelineSteps.length - 1].timestamp,
                    ).getTime() -
                      new Date(timelineSteps[0].timestamp).getTime()) /
                      (1000 * 60 * 60 * 24),
                  )
                : 0}
            </div>
