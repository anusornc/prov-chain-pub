...
\title{ProvChain: A GS1/EPCIS-Oriented Blockchain Framework for Ultra-High Temperature (UHT) Milk Supply Chain Traceability with Semantic Web Integration}
...
\begin{abstract}
Dairy supply chains face persistent challenges in ensuring food safety, maintaining cold chain integrity, and providing end-to-end traceability from farm to consumer. This paper presents \textbf{ProvChain}, a blockchain framework that combines GS1/EPCIS-oriented event modeling with semantic web technologies to support Ultra-High Temperature (UHT) milk traceability. Our framework implements a Write-Ahead Logging (WAL) persistence mechanism for crash recovery and ontology-backed semantic admission through SHACL validation and SPACL-supported reasoning. We introduce a UHT-specific ontology extension that models critical quality parameters including pasteurization temperature (135--150°C), holding time (2--5 seconds), and cold chain monitoring. Through an interactive demonstration featuring 8 supply chain phases from milk collection to retail distribution, we show how a domain package can extend a shared provenance core while reusing the same blockchain and validation infrastructure. The current evaluation reports both a single-node development load test and focused ontology-admission benchmarks. In the development load test, the system achieved 19.58 TPS with 100\% request success, while focused ontology-backed \texttt{add\_block} admission remained in the sub-millisecond to low-millisecond range depending on package and batch size. Our work contributes a proof-of-concept reference implementation that bridges blockchain traceability with ontology-governed validation and standards-facing interoperability for food supply chains.
\end{abstract}
...
% Metrics Box
\node[box, fill=yellow!20] at (0,-2.5) {\textbf{Performance:} 19.58 TPS (dev) $|$ 0.51--1.01ms focused admission $|$ UHT-centered package benchmarks};
...
\begin{highlights}
\item Proof-of-concept blockchain framework for dairy traceability using GS1/EPCIS-oriented event modeling and semantic web technologies
...
\textbf{Contribution 1}: A permissioned blockchain framework with shared-ontology semantic validation and GS1/EPCIS-oriented event modeling, demonstrated through a UHT case study.
...
This paper presented ProvChain, a proof-of-concept blockchain framework for UHT milk supply chain traceability that integrates GS1/EPCIS-oriented event modeling with ontology-governed validation. Our contributions include: (1) a permissioned blockchain framework with reusable semantic architecture; (2) a UHT extension with SHACL validation for critical processing parameters; (3) ontology-backed reasoning capabilities including hasKey-style validation and property-chain inference; (4) a WAL persistence mechanism with atomic writes and crash recovery; and (5) an open-source reference implementation with interactive demonstration and focused benchmark artifacts.
