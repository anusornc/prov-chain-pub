...
\begin{abstract}
Modern supply chains across diverse industries---including food, pharmaceuticals, healthcare, and manufacturing---face persistent challenges in ensuring product integrity, maintaining quality throughout distribution, and providing end-to-end traceability. This paper presents \textbf{ProvChain}, a generalized blockchain framework that integrates GS1 Electronic Product Code Information Services (EPCIS) 2.0 standards with semantic web technologies to enable comprehensive traceability across multiple domains. Our framework features a modular ontology architecture supporting domain-specific extensions including food safety, pharmaceutical compliance, healthcare asset tracking, and automotive parts traceability. The system implements a Write-Ahead Logging (WAL) persistence mechanism ensuring data durability and crash recovery, while incorporating OWL2 ontologies for enhanced semantic validation and reasoning. Through an interactive demonstration featuring Ultra-High Temperature (UHT) milk processing as a representative case study---modeling critical quality parameters including pasteurization temperature (135--150°C), holding time (2--5 seconds), and cold chain monitoring---we validate the framework's capability to capture multiple event types (ObjectEvent, TransformationEvent, AggregationEvent, etc.) with standardized CBV business steps and dispositions. The modular architecture enables straightforward adaptation to other domains by substituting domain-specific ontologies while maintaining the core EPCIS-compliant event processing infrastructure. Performance evaluation demonstrates event processing in 65ms average with full semantic validation. Our work contributes a domain-agnostic reference implementation that bridges the gap between blockchain traceability and global supply chain standards, demonstrating the feasibility of integrating semantic web technologies with distributed ledger systems for enhanced product integrity monitoring and regulatory compliance across diverse industries.
\end{abstract}
...
\begin{highlights}
\item Generalized blockchain framework integrating GS1 EPCIS 2.0 with semantic web technologies for multi-domain traceability
\item Modular ontology architecture supporting food, pharmaceutical, healthcare, and automotive domains
...
\item Interactive demonstration validated across 8 supply chain phases with 7 EPCIS event types
...
ProvChain integrates OWL2 reasoning for semantic inference including: (1) hasKey validation ensuring batch ID uniqueness; (2) property chain inference for supplier relationships; and (3) qualified cardinality validating 4 quality tests per batch. To optimize performance while maintaining semantic expressivity, the framework employs a hybrid reasoning engine that combines a native OWL2 reasoner for complex assertions with specialized, high-performance logic for common supply chain property chains.
...
\textbf{Throughput bottleneck:} At 128 events/s, ProvChain is significantly slower than Hyperledger Fabric (3000+ TPS). This is primarily due to: (1) RDF parsing and serialization overhead (~40\% of processing time); (2) SHACL validation requiring SPARQL queries (~25\%); (3) OWL2 reasoning for each batch (~15\%); and (4) synchronous fsync calls for WAL durability (~20\%). Additionally, the current implementation performs full N-Quads serialization for storage checkpoints, which may introduce latency as the graph grows. Future work includes implementing incremental RDF indexing and delta-based storage to improve I/O efficiency. For context, typical dairy processors generate 50-200 events per minute, making ProvChain suitable for SME deployments but requiring optimization for large-scale industrial operations.
