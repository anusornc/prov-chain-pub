...
% [To be inserted: Visual summary showing]
% 1. Farm to retail supply chain flow
% 2. Blockchain layer with EPCIS events
% 3. Semantic web layer with ontologies
...
We developed a UHT-specific ontology extending EPCIS with domain concepts. The class hierarchy includes:

\begin{itemize}
    \item \texttt{uht:MilkCollectionEvent} (subClassOf \texttt{epcis:ObjectEvent})
    \item \texttt{uht:UHTProcessingEvent} (subClassOf \texttt{epcis:TransformationEvent})
    \item \texttt{uht:QualityTestEvent} (subClassOf \texttt{epcis:ObjectEvent})
    \item \texttt{uht:ColdChainEvent} (subClassOf \texttt{epcis:ObjectEvent})
\end{itemize}
...
SHACL shapes enforce data quality constraints:
...
The \texttt{EpcisEventBuilder} provides a fluent API for creating standards-compliant events:
...
We implemented an interactive demonstration covering 8 supply chain phases: (1) milk collection at farm with temperature recording; (2) cold chain transport to processing facility; (3) laboratory testing for acidity, fat, protein, and bacteria; (4) UHT processing at 137°C for 4 seconds; (5) aseptic packaging into cartons; (6) refrigerated storage at 4°C; (7) distribution to DC; and (8) receipt at distribution center.
