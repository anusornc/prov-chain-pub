...
impl Owl2EnhancedReasoner {
    ...
    pub fn process_owl2_features(&mut self) -> Result<()> {
        ...
        if self
            .base_reasoner
            .config
            .enable_qualified_cardinality_validation
        {
    ...
    pub fn process_has_key_axioms(&mut self) -> Result<()> {
        ...
        // Use a more comprehensive query that directly extracts the property lists
        let query = r#"
            ...
            SELECT ?class ?property WHERE {
                ...
                ?keyList rdf:rest*/rdf:first ?property .
            }
        "#;
    ...
    pub fn process_property_chain_axioms(&mut self) -> Result<()> {
        ...
        let query = r#"
            ...
            SELECT ?superProperty ?property WHERE {
                ?superProperty owl:propertyChainAxiom ?chainList .
                ?chainList rdf:rest*/rdf:first ?property .
            }
        "#;
        ...
        for (super_prop, chain_props) in &self.property_chains {
            info!(
                "Super property: {} has chain properties: {:?}",
                super_prop, chain_props
            );
        }
        Ok(())
    }
    ...
    pub fn process_qualified_cardinality_restrictions(&mut self) -> Result<()> {
        ...
        // Query for owl:qualifiedCardinality and owl:minQualifiedCardinality restrictions
        let query = r#"
            PREFIX owl: <http://www.w3.org/2002/07/owl#>
            PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    ...
    pub fn validate_entity_uniqueness(&self, entity_data: &str) -> Result<ValidationResult> {
        if !self.base_reasoner.config.enable_has_key_validation
            || self.has_key_constraints.is_empty()
        {
...
mod tests {
    ...
    fn test_has_key_constraint_extraction() -> Result<()> {
        ...
        assert!(reasoner
            .get_has_key_constraints()
            .contains_key("http://provchain.org/test-owl2#Batch"));
        if let Some(keys) = reasoner
            .get_has_key_constraints()
            .get("http://provchain.org/test-owl2#Batch")
        {
        ...
        assert!(reasoner
            .get_has_key_constraints()
            .contains_key("http://provchain.org/test-owl2#ProductBatch"));
        if let Some(keys) = reasoner
            .get_has_key_constraints()
            .get("http://provchain.org/test-owl2#ProductBatch")
        {
    ...
    fn test_property_chain_extraction() -> Result<()> {
        ...
        assert!(reasoner
            .get_property_chains()
            .contains_key("http://provchain.org/test-owl2#transitivelySuppliedTo"));
