...
impl PersistentStorage {
    ...
    pub fn open<P: AsRef<Path>>(data_dir: P) -> Result<Self> {
        ...
        fs::create_dir_all(&rdf_data_dir)
            .with_context(|| format!("Failed to create RDF directory: {}", rdf_data_dir.display()))?;
    ...
    fn write_wal_header(&self) -> Result<()> {
        ...
        file.sync_all()
            .context("Failed to sync WAL header")?;
    ...
    fn recover(&self) -> Result<()> {
        ...
        if version != PERSISTENCE_VERSION {
            warn!("WAL version mismatch: expected {}, found {}", PERSISTENCE_VERSION, version);
        }
        ...
        loop {
            ...
            if entry_len == 0 || entry_len > 100_000_000 {
                ...
                corrupt_entries += 1;
                warn!("Invalid entry size: {} at position {:?}", entry_len, reader.stream_position());
                break;
            }
        ...
        info!(
            ...
            entry_count, corrupt_entries, highest_sequence + 1
        );
    ...
    pub fn store_block(
        ...
        let rdf_file_path = self.rdf_data_dir.join(format!("block_{}.ttl", metadata.index));
    ...
    pub fn load_block_data(&self, index: u64) -> Result<String> {
        ...
        fs::read_to_string(&rdf_file_path)
            .with_context(|| format!("Failed to read RDF data for block {} from {}", 
                index, rdf_file_path.display()))
    }
    ...
    pub fn load_all_blocks(&self) -> Result<Vec<BlockMetadata>> {
        ...
        let index_data = fs::read_to_string(&self.chain_index_path)
            .context("Failed to read chain index")?;
        ...
        for line in index_data.lines() {
            ...
            let metadata: BlockMetadata = bincode::deserialize(
                &hex::decode(line)
                    .context("Failed to decode chain index entry")?
...
mod tests {
    ...
    fn test_persistent_storage_basic() {
        ...
        storage.store_block(&metadata, "@prefix ex: <http://example.org/> .\nex:test ex:value 1 .").unwrap();
    ...
    fn test_storage_recovery() {
        ...
        // Create storage and store blocks
        {
            ...
            for i in 0..5 {
                ...
                storage.store_block(&metadata, &format!("@prefix ex: <http://example.org/> .\nex:block{} ex:index {} .", i, i)).unwrap();
            }
        }
