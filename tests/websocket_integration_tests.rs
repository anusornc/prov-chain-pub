    ...
    if let TungsteniteMessage::Text(text) = msg {
        ...
        match parsed {
            WebSocketMessage::Connected { client_id } => {
                ...
                println!(
                    "✅ Connection acknowledgment received with client_id: {}",
                    client_id
                );
            }
...
async fn test_blockchain_event_broadcasting() {
    ...
    broadcaster.broadcast_block_created(1, "test-block-hash-123".to_string(), 5);
...
async fn test_multiple_event_types() {
    ...
    let test_events: Vec<(&str, Box<dyn Fn()>)> = vec![
        ...
        (
            "metrics_update",
            Box::new(|| {
                broadcaster.broadcast_metrics_update(2.5, 10.0, 24.5, "excellent".to_string());
            }),
        ),
    ];
    ...
    for (event_name, broadcast_fn) in test_events {
        ...
        let msg = timeout(Duration::from_secs(5), ws_stream.next())
            .await
            .unwrap_or_else(|_| panic!("Timeout waiting for {} event", event_name))
            .unwrap()
            .unwrap();
        ...
        if let TungsteniteMessage::Text(text) = msg {
            ...
            match parsed {
                WebSocketMessage::Event(event) => match event {
                    BlockchainEvent::TransactionSubmitted { transaction_id, .. }
                        if event_name == "transaction_submitted" =>
                    {
                        assert_eq!(transaction_id, "tx-123");
                        println!("✅ Transaction submitted event received correctly");
                    }
                    BlockchainEvent::ValidationComplete {
                        block_index,
                        is_valid,
                        ..
                    } if event_name == "validation_complete" => {
                        assert_eq!(block_index, 1);
                        assert!(is_valid);
                        println!("✅ Validation complete event received correctly");
                    }
                    BlockchainEvent::IntegrityAlert { level, message, .. }
                        if event_name == "integrity_alert" =>
                    {
                        assert_eq!(level, "warning");
                        assert_eq!(message, "Test alert message");
                        println!("✅ Integrity alert event received correctly");
                    }
                    BlockchainEvent::MetricsUpdate {
                        blocks_per_minute, ..
                    } if event_name == "metrics_update" => {
                ...
                _ => panic!(
                    "Expected Event message for {}, got: {:?}",
                    event_name, parsed
                ),
            }
        } else {
...
async fn test_client_message_handling() {
    ...
    let subscribe_msg = WebSocketMessage::Subscribe {
        events: vec![
            "BlockCreated".to_string(),
            "TransactionSubmitted".to_string(),
        ],
    };

    let msg_text = serde_json::to_string(&subscribe_msg).unwrap();
    ws_stream
        .send(TungsteniteMessage::Text(msg_text))
        .await
        .unwrap();
    ...
    ws_stream
        .send(TungsteniteMessage::Text(ping_text))
        .await
        .unwrap();
...
async fn test_broadcast_to_multiple_clients() {
    ...
    // Broadcast event
    let broadcaster = BlockchainEventBroadcaster::new(state.clone());
    broadcaster.broadcast_block_created(42, "broadcast-test-hash".to_string(), 10);
    ...
    for (i, client) in clients.iter_mut().enumerate() {
        loop {
            let msg = timeout(Duration::from_secs(5), client.next())
                .await
                .unwrap_or_else(|_| panic!("Timeout waiting for event on client {}", i + 1))
                .unwrap()
                .unwrap();
...
async fn test_concurrent_connections_load() {
    ...
    for i in 0..NUM_CLIENTS {
        ...
        let handle = tokio::spawn(async move {
            ...
            let _ack = timeout(Duration::from_secs(10), ws_stream.next())
                ...
                .unwrap_or_else(|_| {
                    panic!(
                        "Timeout waiting for connection acknowledgment on client {}",
                        i
                    )
                })
                .unwrap()
    ...
    assert_eq!(state.client_count(), 0);
    println!(
        "✅ Load test completed: {} concurrent connections handled successfully",
        NUM_CLIENTS
    );
}
...
async fn test_event_broadcasting_performance() {
    ...
    for i in 0..NUM_EVENTS {
        broadcaster.broadcast_block_created(i as u64, format!("perf-test-hash-{}", i), i % 10 + 1);
    }

    let broadcast_duration = start_time.elapsed();

    println!(
        "✅ Broadcasted {} events to {} clients in {:?}",
