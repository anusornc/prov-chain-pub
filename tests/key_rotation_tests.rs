...
mod key_rotation_timing_tests {
    ...
    fn test_should_rotate_key_when_expired() {
        ...
        assert!(
            blockchain.should_rotate_key(),
            "Key should be rotated after interval expires"
        );
    }
    ...
    fn test_should_not_rotate_key_when_fresh() {
        ...
        assert!(
            !blockchain.should_rotate_key(),
            "Key should not be rotated when fresh"
        );
    }
    ...
    fn test_should_rotate_key_at_exact_interval() {
        ...
        assert!(
            blockchain.should_rotate_key(),
            "Key should rotate at exact interval"
        );
    }
    ...
    fn test_days_since_key_rotation() {
        ...
        for (days_ago, expected) in test_cases {
            ...
            assert!(
                (result - expected as i64).abs() <= 1,
                "Days since rotation should be approximately {} (got {})",
                expected,
                result
            );
        }
    ...
    fn test_future_rotation_time() {
        ...
        assert!(
            !blockchain.should_rotate_key(),
            "Freshly rotated key should not need rotation"
        );
        ...
        assert!(
            !blockchain.should_rotate_key(),
            "Should not rotate 1 day before interval"
        );
    }
}
...
mod rotation_interval_configuration_tests {
    ...
    fn test_custom_rotation_interval_short() {
        ...
        assert!(
            blockchain.should_rotate_key(),
            "Should rotate with custom 30-day interval"
        );
    }
    ...
    fn test_custom_rotation_interval_long() {
        ...
        assert!(
            blockchain.should_rotate_key(),
            "Should rotate with custom 365-day interval"
        );
    ...
    fn test_minimum_rotation_interval() {
        ...
        assert!(
            blockchain.should_rotate_key(),
            "Should rotate with 1-day minimum interval"
        );
    }
    ...
    fn test_zero_rotation_interval() {
        ...
        assert!(
            blockchain.should_rotate_key() || blockchain.days_since_key_rotation() >= 0,
...
mod rotation_tracking_tests {
    ...
    fn test_rotation_initialization() {
        ...
        assert!(
            time_diff < 5,
            "Initial rotation time should be close to now"
        );
        assert_eq!(
            blockchain.key_rotation_interval_days, 90,
            "Default interval should be 90 days"
        );
    }
...
mod rotation_warning_tests {
    ...
    fn test_approaching_rotation_warning() {
        ...
        for (days_ago, should_warn) in test_cases {
            blockchain.last_key_rotation = Utc::now() - Duration::days(days_ago);
            let days_left =
                blockchain.key_rotation_interval_days as i64 - blockchain.days_since_key_rotation();

            let is_approaching = days_left <= 7; // Warn within 7 days
            assert_eq!(
                is_approaching, should_warn,
                "Warning at {} days should be {}",
                days_ago, should_warn
            );
        }
    }
    ...
    fn test_critical_rotation_period() {
        ...
        for (days_ago, is_critical) in test_cases {
            ...
            assert_eq!(
                result, is_critical,
                "Critical status at {} days should be {}",
                days_ago, is_critical
            );
        }
    }
    ...
    fn test_rotation_overdue() {
        ...
        for overdue in overdue_days {
            ...
            assert!(
                blockchain.should_rotate_key(),
                "Should be overdue by {} days",
                overdue
            );

            let days_overdue = blockchain.days_since_key_rotation() - 90;
            assert_eq!(days_overdue, overdue, "Should be {} days overdue", overdue);
        }
    }
}
...
mod edge_cases_tests {
    ...
    fn test_rotation_with_leap_year() {
        ...
        assert!(
            blockchain.should_rotate_key(),
            "Should rotate after leap year interval"
        );
    }
...
mod integration_tests {
    ...
    fn test_full_rotation_workflow() {
        ...
        assert!(
            !blockchain.should_rotate_key(),
            "Should not rotate immediately after rotation"
        );
        assert_eq!(blockchain.days_since_key_rotation(), 0);
    }
    ...
    fn test_rotation_schedule_consistency() {
        ...
        for cycle in 0..5 {
            ...
            assert!(
                blockchain.should_rotate_key(),
                "Cycle {}: Should rotate at 90 days",
                cycle
            );
            ...
            assert!(
                !blockchain.should_rotate_key(),
                "Cycle {}: Should not rotate immediately after",
                cycle
            );
            ...
            assert!(
                !blockchain.should_rotate_key(),
                "Cycle {}: Should not rotate mid-cycle",
                cycle
            );
        }
    }
