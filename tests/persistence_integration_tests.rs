...
use provchain_org::storage::rdf_store::{RDFStore as RdfStore, StorageConfig};
...
fn test_concurrent_access() {
    ...
    let store: Arc<Mutex<RdfStore>> =
        Arc::new(Mutex::new(RdfStore::new_persistent(&storage_path).unwrap()));
